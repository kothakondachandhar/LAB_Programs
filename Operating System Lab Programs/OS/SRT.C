/*PROGRAM TO IMPLEMENT SCHEDULING USING
  "SHORTEST JOB FIRST" TECHNIQUE WITH PRE-EMPTION*/
...
main()
{
 ...
 struct process
 {
  ...
  /*HERE rt IS USED TO STORE REMAINING TIME*/
  float tar;
 }p[10],t[10],temp;
 ...
 while(1)
 {
  ...
  if( p[j].rt<p[i].rt || ( p[j].rt==p[i].rt && p[j].at<p[i].at ) )
  i=j;
 ...
 clrscr();
 printf("DETAILS COMPUTED AFTER SCHEDULING USING SJF ( PRE-EMPTIVE )\n\n");
